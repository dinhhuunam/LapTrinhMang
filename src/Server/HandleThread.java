...
//	luồng xử lý (thread) độc lập, được sử dụng để xử lý kết nối với mỗi Client.
	...
	@Override
	public void run() {
//		Phương thức này bắt đầu bằng việc gửi một gói tin chứa ID của phòng tới Client, 
//		sau đó bắt đầu vòng lặp vô hạn (while (true)) để đọc các gói tin đến từ Client.
		super.run();
...
//		xử lý việc đọc dữ liệu từ Client. Dữ liệu đọc được là các gói tin được gửi từ Client. 
//		Mỗi gói tin có một thẻ (tag) xác định loại gói tin, và dựa vào thẻ này, Server xử lý các tác vụ khác nhau.
//		Các tác vụ như đăng ký tên tài khoản, xác định người chơi đầu tiên, đánh bài, truyền tin nhắn, 
//		xác định người chiến thắng, v.v. đều được xử lý trong phương thức doRead().
		try {
...
//		được sử dụng để gửi các gói tin đến Client. 
//		Điều này cho phép Server gửi dữ liệu hoặc thông báo tới Client.		
		try {
